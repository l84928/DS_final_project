...
if checkpoint_dir is None:
    ckpt_dir = 'C:/Users/Ying Jhu/Desktop/BertGCN-main/checkpoint/{}_{}'.format(bert_init, dataset)
else:
...
corpus_file = 'C:/Users/Ying Jhu/Desktop/BertGCN-main/data/corpus/'+dataset+'_shuffle.txt'
...
metrics={'acc': Accuracy(),'nll': Loss(th.nn.CrossEntropyLoss())}
...
def log_training_results(trainer):
    ...
    logger.info("\rEpoch: {}  Train acc: {:.4f} loss: {:.4f}  Val acc: {:.4f} loss: {:.4f}  Test acc: {:.4f} loss: {:.4f}".format(trainer.state.epoch, train_acc, train_nll, val_acc, val_nll, test_acc, test_nll))
    if val_acc > log_training_results.best_val_acc:
        logger.info("New checkpoint")
        th.save({'bert_model': model.bert_model.state_dict(),'classifier': model.classifier.state_dict(),'optimizer': optimizer.state_dict(),'epoch': trainer.state.epoch,},
